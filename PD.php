...
$query="SELECT DISTINCT * FROM regdoners";
...
$query="UPDATE regdoners SET Status='{$status}' WHERE ID='{$ID}'";
$result=mysqli_query($con,$query);
}
elseif(isset($_GET["ID"]) && isset($_GET["control"])){
$ID = $_GET["ID"];
if($_GET["control"]=="remove"){
$query="DELETE FROM regdoners WHERE ID='{$ID}'";
...
<head>
    ...
    <title><?php echo $expr['List Of Blood Donors']?></title>
    <meta content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" name="viewport">
    <meta http-equiv="refresh" content="2;url=PD.php" />
...
<div class="page_info">
            <div class="container">
                <div class="row">
                    ...
                    <div class="col-md-4 col-sm-4 col-xs-6" style="text-align:right;"><?php echo $expr['Profile Admin']?><span class="sep"> / </span><span class="current"><?php echo $expr['List Of Blood Donors']?></span></div>
                </div>
            </div>
        </div>
</section>
<div class="container">
  <h2><?php echo $expr['List Of Blood Donors']?></h2>
  <p><?php echo $expr['Search For Any Blood Type Or Name Of Doners Or City Of Doners']?></p>  
...
if($row['Status'] == 1){
     $action = "<a href='PD.php?ID={$row['ID']}&status=deactive' class='btn btn-danger'>{$expr['Disable']}</a>";
}
elseif($row['Status'] == 0){
    $action = "<a class='btn btn-success' href='PD.php?ID={$row['ID']}&status=active' >{$expr['Enable']}</a>";
}
echo"<tr><td>".$num++."</td><td>".$row['Name']."</td><td>".$row['BT']."</td><td>".$row['NumberUnit']." Units"."</td><td>".$row['Address']."</td><td>".$row['Gender']."</td><td>".$row['Age']."</td><td>".$row['SS']."</td><td>".$row['Nphone']."</td><td>".$row['Email']."</td><td>".$row['AdditonalInfo']."</td><td>".$row['ContactInfo']."</td><td>"."&nbsp;$action&nbsp;<a href='PD.php?ID={$row['ID']}&control=remove' class='btn btn-warning'>{$expr['Remove']}</a>"."</td></tr>";
}  
//echo "</table>";
}
?>
  ...
  <p><?php echo $expr['Control All Persons Who Registers On List Of Donors']?></p>
</div>
